...
5. Chat send: `POST /chat/messages` on `TwitchService` → `TwitchChatService` → Helix `POST /helix/chat/messages`, using the Bot token. Requires the `user:write:chat` scope, which is configured on the `Web` side (`BotScopes` in `appsettings.*.json`), not `TwitchService`'s.
