...
5. Chat auth model: both the `channel.chat.message` EventSub subscription (`TwitchWebSocketService.SubscribeToChannelChatMessages`) and chat send (`TwitchChatService.SendMessageAsync` → Helix `POST /helix/chat/messages`) authenticate via the app-access client (`TwitchClientAppAccess`/`TwitchAppAuthHandler`, `client_credentials`) rather than the bot's own user token. This relies on the broadcaster's one-time `channel:bot` grant and the bot's one-time `user:bot` grant (configured as `BroadcasterScopes`/`BotScopes` in `Web`'s `appsettings.*.json`) — Twitch remembers that consent server-side, so no ongoing bot user-token refresh is needed for these two calls. The stored Bot token is still used as a proxy for "the bot has completed OAuth at least once" (gating whether to attempt the subscription) and to resolve the bot's user id for `sender_id`. `channel.channel_points_custom_reward_redemption.add` is unaffected and still authenticates on the Broadcaster's user token.

### Config

Twitch client ID/secret and OAuth scopes live in each service's `appsettings*.json` under a `Twitch` (or legacy `TwitchLib`, in `Web`) section — `appsettings.Development.json` is gitignored per project, so scope changes made locally won't show up in `git status`/diffs. The README's scopes list predates the Helix chat migration (still lists IRC-era `chat:edit`/`chat:read`/`whispers:read`); treat `Web`'s `appsettings.*.json` `BotScopes`/`BroadcasterScopes` as the current source of truth instead. `Host`'s `appsettings.prompts.json` holds the OpenAI system prompts for the (currently dormant) AI layer.
